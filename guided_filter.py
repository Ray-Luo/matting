import numpy as np
import cv2 as cv
# from cv2.smooth import cv.blur
# from cv2.image import to_32F
...
class GrayGuidedFilter:
    ...
    def filter(self, p):
        ...
        """
        # step 1
        meanI  = cv.blur(I=self.I, r=self.radius)
        meanp  = cv.blur(I=p, r=self.radius)
        corrI  = cv.blur(I=self.I * self.I, r=self.radius)
        corrIp = cv.blur(I=self.I * p, r=self.radius)
        # step 2
        ...
        b      = meanp - a * meanI
        # step 4
        meana  = cv.blur(I=a, r=self.radius)
        meanb  = cv.blur(I=b, r=self.radius)
        # step 5
...
class MultiDimGuidedFilter:
    ...
    def filter(self, p):
        ...
        meanI = cv.blur(I=self.I, r=self.radius) # (H, W, C)
        meanp = cv.blur(I=p_, r=self.radius) # (H, W, 1)
        ...
        corrI_ = cv.blur(I=corrI_, r=self.radius)
        ...
        corrIp = cv.blur(I=self.I*p_, r=self.radius) # (H, W, C)
